...
pub async fn handle(
    ...
    info!("[{uuid}] Response_topic: {}", &response_topic);
    ...
    info!(
        "[{uuid}] Registering account: {:?} with topic: {} at project: {}. Scope: {:?}",
        &client_data.id,
